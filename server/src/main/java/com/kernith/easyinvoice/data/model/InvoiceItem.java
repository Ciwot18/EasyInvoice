...
@Table(
        name = "invoice_items",
        uniqueConstraints = {
                @UniqueConstraint(name = "uq_invoice_items_invoice_position", columnNames = {"invoice_id", "position"})
        },
        indexes = {
...
public class InvoiceItem {
    ...
    public InvoiceItem(Invoice invoice, QuoteItem quoteItem) {
        this.invoice = invoice;
        if (quoteItem != null) {
            this.position = quoteItem.getPosition();
            this.description = quoteItem.getDescription();
            this.notes = quoteItem.getNotes();
            this.quantity = quoteItem.getQuantity();
            this.unit = quoteItem.getUnit();
            this.unitPrice = quoteItem.getUnitPrice();
            this.taxRate = quoteItem.getTaxRate();
            this.discountType = mapDiscountType(quoteItem.getDiscountType());
            this.discountValue = quoteItem.getDiscountValue();
        }
    ...
    private BigDecimal applyDiscount(BigDecimal lineSubtotal) {
        ...
        DiscountType effectiveType =
                discountType == null ? DiscountType.NONE : discountType;
    ...
    private DiscountType mapDiscountType(DiscountType quoteDiscountType) {
        ...
        return DiscountType.valueOf(quoteDiscountType.name());
    }
}
